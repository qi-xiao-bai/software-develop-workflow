...
This order usually produces documents that are easier to scan because they move from broad understanding to concrete execution detail.
...
If the reader still needs to infer the purpose of every file from the name alone, the document is too thin.
...
The more invisible the runtime behavior is from a static file tree, the more explicit the document needs to be.
